<template>
  <div
    :class="[config.wrapper, to && config.to]"
    v-bind="$attrs"
  ...
  >
    ...
    <UIcon v-if="icon" :name="icon" :class="config.icon.base" />

    <UIcon
      v-if="!!to && target === '_blank'"
      :name="config.externalIcon.name"
      :class="config.externalIcon.base"
    />
...
const target = computed(
  () =>
    props.target ||
    (props.to && props.to.startsWith("http") ? "_blank" : undefined)
);
</script>
